...
#### 3. Replace 0 in rarefied-table with non-rarefied relative abundance or adding 10-8 if remain 0 before rarefaction####
 ...
 #### Part 2: Growth-based competitiveness-invulnerability trade-off ####
  ...
  for (i in 1:length(temp)){
    t=read.table(temp[i], header=T, row.names = 1)
    x=vegdist(t(t), method="bray", binary=FALSE, diag=FALSE, upper=FALSE, na.rm = FALSE) #Calculate Bray-Curtis disimilarity between each T12 and T0
    tt=as.matrix(x)[,1]
  ...
  mtext("Predation dilution facotrs", outer=T,side=1, line=1, cex=0.9)
  mtext("Bray-Curtis disimilarity Between T0 and T12", outer=T, side=2, line=1, cex=0.9)
  
  Sample=sapply(strsplit(temp, "_", fixed = TRUE), "[", 3)
  legend(1,0.55, 
         xpd = "NA",
         legend = Sample, 
         pch=PCH, col=COL, cex=0.45)
  ########################### End of Part 6 ################################################
