...
#Jinny Wu Yang, Feng-Hsun Chang, Yi-Chun Yeh, An-Yi Tsai, Kuo-Ping Chiang, Gwo-Ching Gong, Chih-hao Hsieh
#jinnyang@umich.edu
...
#### Fit rank-nomalize RAD to zipfbrot model and calculate RAD decay coefficient ####
...
LMMEst=function(x,y,r){
  ...
  names(LMM)=c("Value" ,"Std. Error","DF","t-value","p-value","Fix_intercept","Fix_slope")  
  return(LMM)
}
...
DiversityPlot_Combine_Protists=function(l,x,Tr,Ylim,LMMTable){
  ...
  if (LMMTable$p.value<=0.05){
    LTY=1
  }else{
    LTY=2
  }
  if (LMMTable$p.value<=0.001){
    p="<0.001"
  }else{
    p=LMMTable$p.value
  }
